...
module.exports = async (req, res, next) => {
  try {
    ...
    if (!isAccessTokenValidate) {
      ...
      // console.log(jwt.verify(refreshtoken, process.env.SECRETKEY));
      const { userId } = jwt.verify(refreshtoken, process.env.SECRETKEY);
      // console.log(userId);
      ...
      /**토큰이 모두 유효한 경우 */
      const { userId } = jwt.verify(accesstoken, process.env.SECRETKEY);
      const user = await Users.findByPk(userId);
      res.locals.user = user;
    }
